...
print("Using torch float16, if need more precision need to change custom_fwd")
class InvertibleCheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, fn, fn_inverse, keep_input, num_bwd_passes, preserve_rng_state, num_inputs, *inputs_and_weights):
        ...
        with torch.no_grad():
            ...
            # print([element.dtype for element in inputs])
            outputs = ctx.fn(*x)
        ...
        # BRAM MODIFICATION BELOW
        # THIS FIXED SOME ERROR I DON'T REMEMBER AND THEN REVERTING TO ABOVE
        # WAS THE FINAL DEBUG NEEDED SO IDK
        # detached_outputs = tuple([element.detach() for element in outputs])

        # clear memory from inputs
        # BRAM NOTE: This is what's destroying for later feeds
            ...
            with torch.random.fork_rng(devices=rng_devices, enabled=ctx.preserve_rng_state):
                ...
                with torch.no_grad():
                    ...
                    for element_original, element_inverted in zip(inputs, inputs_inverted):
                        ...
                        # print(element_original, element_inverted)
